...
<script type="text/javascript">
	var eee;
        $(function () {
			...
			submitHandler : function() {
				...
				$.post("${pageContext.request.contextPath}/system/User_roleendadd.action?role.rolename="+ encodeURI(encodeURI($("#rolename").val()))
		        +"&role.roledescribe="+ encodeURI(encodeURI($("#roledescribe").val()))
		        +"&role.rolerank="+ encodeURI(encodeURI($("#rolerank").val())),null,function(){
		        alert('添加成功!');
		        parent.m.hide();
	            parent.g.loadData();
		       });
	...
	var flg = true;
	function check() {	    
		$.post("${pageContext.request.contextPath}/system/User_rolecheck.action?role.rolename="
			+ encodeURI(encodeURI($("#rolename").val())),null, function(data) {
			if (data == "fail") {
				$("#result").html("该角色已经存在!");
				$("#result").show();
				flg = false;
	...
	<form name="form1" method="post" action="" id="form1">
		...
		<table cellpadding="0" cellspacing="0" class="l-table-edit">
			<tr>
				...
				<td align="left" class="l-table-edit-td"><input name="rolename"
					type="text" onblur="check()" id="rolename" ltype="text" placeholder="请输入角色名称"
					validate="{required:true,minlength:1,maxlength:20}" /></td>
				<td align="left"></td>
			</tr>
			<tr>
				<td align="right" class="l-table-edit-td">角色描述:</td>
				<td align="left" class="l-table-edit-td"><input name="roledescribe"type="text"  id="roledescribe" ltype="text" placeholder="请输入角色描述" validate="{required:true,minlength:1,maxlength:20}" /></td>
				<td align="left"></td>
			</tr>
			<tr>
				<td align="right" class="l-table-edit-td">等级:</td>
