...
var m;	
   function passaudit(){
    ...
    $(rows).each(function (){ 
    　　if(this.status!=1){
         if(parseFloat(this.totalcredits)>=parseFloat(this.standardcredits)){
            str += this.id+",";
         }
       }   
    });
    if(str.length==0){
      alert("请重新查看各学生审核状态或已修学分！");
    }else{
       $.post("${pageContext.request.contextPath}/system/Manage_graduatestuauditagree.action?ids="+str,null,function(){
         alert('审核成功！');
   ...
   function refuseaudit(){
    ...
    　　       if(this.status!=1&&this.status!=2){
         if(parseFloat(this.totalcredits)>=parseFloat(this.standardcredits)){
            $.ligerDialog.confirm('学生护照号：'+this.passport+'已修够学分，确认审核不通过？', function (yes){
              if(yes){
                str += this.id+",";
              }
            });
    ...
    if(str.length==0){
      ...
      $.post("${pageContext.request.contextPath}/system/Manage_graduatestuauditrefuse.action?ids="+str,null,function(){
                alert('批复成功！');
                window.location.reload();
      });
    }
