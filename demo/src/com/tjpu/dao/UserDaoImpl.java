...
public class UserDaoImpl implements UserDao {
	...
	public Dept loaddeptbyid(int deptid) {		 
        String querystring ="from Dept d where d.deptid=:deptid";
        Query query = sessionFactory.getCurrentSession().createQuery(querystring);
	...
	public Dept loaddeptbyresponsible(String teachername) {		 
        String querystring ="from Dept d where d.deptresponsible=:deptid";
        Query query = sessionFactory.getCurrentSession().createQuery(querystring);
        query.setString("deptid", teachername);
        List<Dept> dept = (List<Dept>)query.list();
        if (dept==null) {
			return null;
		}else {
	...
	public void deptdelete(int deptid) {
		...
		Dept dept = (Dept)sessionFactory.getCurrentSession().load(Dept.class, deptid);
		String queryString = "from Teacher t left join fetch t.dept d where d.deptid=:deptid";
		Query query = sessionFactory.getCurrentSession().createQuery(queryString);
		query.setInteger("deptid", deptid);
	...
	public List<Dept> loaddept() {
		// TODO Auto-generated method stub
		String querysString = "from Dept d ";
		Query query = sessionFactory.getCurrentSession().createQuery(querysString);
		List<Dept> depts = (List<Dept>)query.list();
		return depts;
	}
	...
	public List<FileRecord> loadRecords(int lower, int upper) {
		String queryString = "from FileRecord r order by r.serial desc ";
		Query query = sessionFactory.getCurrentSession().createQuery(queryString);
		query.setFirstResult(lower);
		query.setMaxResults(upper);
		List<FileRecord> records = (List<FileRecord>) query.list();
		return records;
	}
	
	public Integer recordsCount () {
		String hql = "select count(*) from FileRecord";
		Query query = sessionFactory.getCurrentSession().createQuery(hql);
		return ((Number) query.uniqueResult()).intValue();
	}

	@Override
	public List<FileRecord> fuzzyRecordResult(String keyword, int lower,
			int upper) {
		String hql = "from FileRecord s where s.operaterName like :t1 " 
		+ "or s.fileType like :t2 or s.year like :t3  or s.serial like :t4 " 
		+ "or s.operaterNum like :t5 or s.date like :t6 or s.fileName like :t7 order by s.serial desc";
		Query query = sessionFactory.getCurrentSession().createQuery(hql);
		String text =  "%" + keyword + "%";
		query.setMaxResults(upper);
		query.setFirstResult(lower);
		for (int i = 1; i < 8; ++i) {
			query.setString("t" + i, text);
		}
	...
	public Integer fuzzyLogsCount(String keyword) {
		String hql = "select count(*) from FileRecord s where s.operaterName like :t1 " 
				+ "or s.fileType like :t2 or s.year like :t3  or s.serial like :t4 or s.operaterNum like :t5 or s.date like :t6";
		Query query = sessionFactory.getCurrentSession().createQuery(hql);
		String text =  "%" + keyword + "%";
		for (int i = 1; i < 7; ++i) {
			query.setString("t" + i, text);
