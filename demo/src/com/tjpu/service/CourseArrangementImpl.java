...
public class CourseArrangementImpl implements CourseArrangement{
	...
	public void SetUable (List<ChosenCourseModel> elective) {
		int num = fullTable.size();
		for (ChosenCourseModel i : elective) {
			i.setNum(String.valueOf(num));
			num++;
		}
		this.unable.addAll(elective);
	}
		...
		WritableWorkbook wwb;
		 try{  
	            ...
	            for (ClassroomModel r : rooms) {
	            	ws.addCell(new Label(col, row, r.getAddress(), cellFormat));
	            	col ++;
	            }
            				...
            				for (ChosenCourseModel course : out)
            					...
            					&& course.getRoom().getNum().equals(rooms.get(r).getNum())) {
            						...
            						if (course.getCourse() != null && course.getTeacher() != null) {
            							...
            							String weeks = String.format("%d-%d周", course.getBeginWeek(), course.getEndWeek());
            							content = String.format("%s\n%s%s\n%s",
            									course.getCourse().getCoursename(), classname, course.getTeacher().getTeachername(), weeks);
            						}
            						ws.mergeCells(col, row, col, row + 2);
	            ...
	            for (int r = 0; r < rooms.size(); ++r) {
            		...
            		for (int d = 0; d < weekday; ++d){
            			for (int s = 0; s < slot; ++s){
            				boolean find = false;
            				for (ChosenCourseModel course : out){
            					if (course.getWeekday() == null || course.getSlot() == null || course.getRoom() == null){
            						continue;
            					}
            					if (course.getWeekday() == d && course.getSlot() == s 
            					&& course.getRoom().getNum().equals(rooms.get(r).getNum())) {
            				...
            				if (!find) {
            					ws.mergeCells(col, row, col, row + 2);
        						ws.addCell(new Label(col, row, "", cellFormat));
        						ws.setRowView(row, 1500);
        						row += 3;
            				}
            			}
	...
	public CourseArrangementImpl(List<ChosenCourseModel> courses,
		...
		this.rooms = rooms;
		fullTable.addAll(this.courses);
		fullTable.addAll(getEmptyPlace2());
	}
	
	private List<ChosenCourseModel> getEmptyPlace2(){
			...
			for (int d = 0; d < weekday; ++d){
				for (ClassroomModel r : rooms) {
					...
					for (ChosenCourseModel c : courses){
						if (c.getRoom() == null)
							continue;
						if (c.getRoom().getNum().equals(r.getNum())) {
							find = true;
							break;
						}
					}
					if (!find) {
						ChosenCourseModel hehe = new ChosenCourseModel();
						hehe.setSlot(s);
						hehe.setWeekday(d);
						hehe.setRoom(r);
					}
	...
	private int getOffset (ChosenCourseModel c, List<ChosenCourseModel> where){
		for (int i = 0; i < where.size(); ++i){
			/*if (c.equals(where.get(i)))
					return i;*/
			if (c.getNum().equals(where.get(i).getNum()))
					return i;
		}
		return -1;
	}
	...
	private void swap (List<ChosenCourseModel> where1, int offset1, List<ChosenCourseModel> where2, int offset2) {
		...
		/*ChosenCourseModel k = where1.get(offset1);
		ChosenCourseModel p = where2.get(offset2);
		
		int ts = k.getSlot();
		int td = k.getWeekday();
		
		
		where1.get(offset1).setSlot(p.getSlot());
		where1.get(offset1).setWeekday(p.getWeekday());
		
		where2.get(offset2).setSlot(ts);
		where2.get(offset2).setWeekday(td);*/
		
		CourseModel ct = null;
		if (where1.get(offset1).getCourse() != null)
			ct = new CourseModel(where1.get(offset1).getCourse());
		
		TeacherModel tt = null;
		if (where1.get(offset1).getTeacher() != null)
			tt = new TeacherModel(where1.get(offset1).getTeacher());
		
		List<ClassesModel> lct = new ArrayList<ClassesModel>();
		if (where1.get(offset1).getClasses() != null && where1.get(offset1).getClasses().size() > 0)
			lct.addAll(where1.get(offset1).getClasses());
		
		String semesterid = where1.get(offset1).getSemesterId();
		
		where1.get(offset1).setSemesterId(where2.get(offset2).getSemesterId());
		...
		if (where2.get(offset2).getClasses() != null && where2.get(offset2).getClasses().size() > 0)
		...
		where2.get(offset2).setSemesterId(semesterid);
	}
	...
	private void classUniquencess(int courseOffset, Spacetime st) {
		...
		for (int i = 0; i < courses.size(); ++i){
			for (ClassesModel k : classes){
				for (ClassesModel j : courses.get(i).getClasses()){
		...
		for (int i = 0; i < courses.size(); ++i){
			String courseNum = courses.get(i).getCourse().getNum();
			String courseNum2 = courses.get(courseOffset).getCourse().getNum();
			Integer classid1 = courses.get(i).getClasses().get(0).getId();
			Integer classid2 = courses.get(courseOffset).getClasses().get(0).getId();
			if (i != courseOffset && courseNum.equals(courseNum2) && classid1.equals(classid2)){
		...
		for (int i = 0; i < sameCourseOffset.size(); ++i)
			for (int r = 0; r < rooms.size(); ++r)
				for (int s = 0; s < slot; ++s)
					noPlace(sameCourseOffset.get(i), r, st.getDay(), s);
		...
		if(st.getDay() != 0){
			for (int i = 0; i < sameCourseOffset.size(); ++i) {
				for (int r = 0; r < rooms.size(); ++r){
					for (int s = 0; s < slot; ++s) {
						noPlace(sameCourseOffset.get(i), r, st.getDay() - 1, s);
					}
				}
			}
		}
