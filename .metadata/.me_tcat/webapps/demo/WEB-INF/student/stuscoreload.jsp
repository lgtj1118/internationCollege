...
<script type="text/javascript">  
	...
	function f_initGrid() {
		g = manager = $("#maingrid").ligerGrid(
				...
				columns : [{display : '序号',    name : 'id',       width : 60,    hide:true,type : 'int'}, 
				           {display : '学期',    name : 'semester', minWidth : 160, editor: { type: 'text'}} ,
				           {display : '课程名称', name : 'coursename',minWidth : 120,editor: { type: 'text'}}, 
				           {display : '护照号',    name : 'passport',     width : 120,   editor: { type: 'text'}}, 
				           ...
				           {display : '成绩',    width : 80,columns:[{
				                     ...
				                     { display:'作业', name:'homework',   editor:{type:'text'}},
   ...
   function f_search(){
      ...
      if (semester.length==0&&classid.length==0&&(studentid.length==0||studentid==null)&&coursenum==null) {
			$("#maingrid").ligerGrid({
					url : '${pageContext.request.contextPath}/system/Manage_allstuscorelist.action'
			   })
	  } else {
	     var str = "${pageContext.request.contextPath}/system/Manage_searchstuscorelist.action?semChosenCourse.semester="
	     +semester+"&cls.id="+classid+"&stu.passport="+studentid
